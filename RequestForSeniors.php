...
<header>
  ...
  <div class="navbar-collapse collapse">
   ...
   <div class="collapse navbar-collapse center_btn" >
		<a href="logout.php" id="btn-logout" class="btn navbar-btn navbar-right log-out" >Log Out</a>
	</div>
  </div>
</nav>
<!--Mask-->
...
<div class="card">
  ...
  <div class="card-body" >
    ...
    <!-- Button -->
    <a  id="driver" class="btn purple-gradient" data-toggle="modal"  data-target="#requestForm">Book Now !</a>
...
<div class="card">
  ...
  <div class="card-body">
	...
	<p class="grey-text">Emotional support and companionship for seniors who are at home.</p>
     <!-- Button -->
    <a id="companion" class="btn purple-gradient" data-toggle="modal"  data-target="#requestForm">Book Now ! </a>
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <!-- Button -->
    <a  id="meal" class="btn purple-gradient" data-toggle="modal"  data-target="#requestForm">Book Now !</a>
...
<div class="card">
  ...
  <div class="card-body">
   ...
   <!-- Button -->
    <a id="cleaning" class="btn purple-gradient" data-toggle="modal"  data-target="#requestForm">Book Now !</a>
...
<div class="col-lg-10 col-lg-offset-1 col-md-12 col-md-offset-0 col-sm-12 col-xs-12 row justify-content-center">
<div style="text-align:center">
<h5><b>Sort by:</b></h5>
<form action="" method="post">
<input type="hidden" id="modal_value" name="modal_value" value="">
    <label class="radio-inline">
      <input type="radio" name="optradio" checked value="service">Service ID
    </label>
    <label class="radio-inline">
...
if ($answer == "service")
 $sql_select_service = "SELECT requestID, date, time,status,service_type, sp_ID FROM servicerequest WHERE username ='$currentUser' ORDER BY requestID desc" ;
if ($answer == "date")
...
if ($result_select_service = $con->query($sql_select_service)) {
	...
	if ($row_count_select_service>0) {
		...
		while($row_select_service=mysqli_fetch_assoc($result_select_service)) {
			...
			$notes_selected_service[$i] = $row_select_service['notes'];
			$status_selected_service[$i] = $row_select_service['status'];
			$service_type_selected_service[$i] = $row_select_service['service_type'];
...
if ($row_count_select_service == 0) {
			...
			document.getElementById("modal_value").value =';echo "$i";echo'
			</script>';
					echo'
...
<div class="view" id="card-hover" >
  ...
  <div class="card-img-overlay white-text zoomed card-style">';
	...
	echo'</div> 
	<button type="submit" data-toggle="modal" data-target="#myModal" data-id="$requestID_selected_service[$i]" id="openModal" class=" btn purple-gradient" style="margin-top:50%;width:100%;display:block;margin-left:0;
margin-right:0;
padding-left:0;
...
document.getElementById('cleaning').onclick = function() {
   document.getElementById("service_type").value = "Cleaning";
}
document.getElementById('companion').onclick = function() {
   document.getElementById("service_type").value = "Companion";
}
document.getElementById('meal').onclick = function() {
   document.getElementById("service_type").value = "Meal Preparation";
}
...
$(document).ready(function() {
   $(".openModal").click(function() {
     $("#namethis").text($(this).attr('data-id'));
